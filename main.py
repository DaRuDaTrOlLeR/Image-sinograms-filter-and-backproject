...
def buildSinogram( image, sinoRows ):
    ...
    #to fix any wrong orientation due the different libraries used in the code we flip original image
    #image = np.flip(image, axis=None)
    ...
    angles = np.linspace(0, np.pi, numSinoAngles, endpoint=False)

    # Iterate over each angle and compute the sinogram
    for i, angle in enumerate(angles):
        
        # Rotate the image by the current angle
        rotated_image = ndimage.rotate(image, np.degrees(-angle), reshape=False)
        
        # Compute the sum of the columns in the rotated image
        col_sums = np.sum(rotated_image, axis=0)
        
        # Copy the column sums into the corresponding row of the sinogram
        sino[i,:] = col_sums

        #sino = np.flip(sino, axis=None)
...
def computeFilteredSinogram( sino ):
    ...
    # Pad the sinogram to the next power of 2
    pad_len = 2 ** int(np.ceil(np.log2(sino.shape[1])))
    sino_padded = np.pad(sino, [(0, 0), (0, pad_len - sino.shape[1])], mode='constant')
    
    # Compute the Fourier transform of the padded sinogram
    sino_ft = np.fft.fft(sino_padded, axis=1)
    
    # Compute the frequency values for the Fourier transform
    freqs = np.fft.fftfreq(sino_padded.shape[1])
    
    # Apply the Ram-Lak filter to the Fourier transform
    ramp_filter = np.abs(freqs)
    ramp_filter[freqs == 0] = 0
    sino_ft_filtered = sino_ft * ramp_filter
    
    # Compute the inverse Fourier transform of the filtered sinogram
    sino_filtered = np.real(np.fft.ifft(sino_ft_filtered, axis=1))[:, :sino.shape[1]]
...
def computeBackprojection( sinogram ):
    ...
    # Compute the angles at which the sinogram was taken
    angles = np.linspace(0, np.pi, sinoRows, endpoint=False)

    # Iterate over each angle and compute the backprojection
    for i, angle in enumerate(angles):
        
        # Create an image of the same size as the sinogram with the current row repeated
        row_image = np.tile(sinogram[i], (dim, 1))
        
        # Rotate the row image by the current angle
        rotated_row_image = ndimage.rotate(row_image, np.degrees(angle), reshape=False, order=1)
        
        # Add the rotated row image to the backprojection
        image += rotated_row_image
    
    # Scale the backprojection correctly
    image *= np.pi / (2 * sinoRows)
